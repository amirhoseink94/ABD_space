...
namespace ABD{
	enum SHAPE{SPHERE, PLANE, PLANE_STIP, PLANE_CURVED, MOB};
	class Body
	{
		public:
		// all points, segments, faces

		vector<Particle*> points;				// points are the essential material
		...
		Particle* center; 							// the centere of the sphere
		double radius;
		...
		Body(bool /* movable or not */,
				 ABD::SHAPE /* shale */,
				 vec /* position */,
			 	 double /* radius */,
			 	 double /* overal weight */);
		...
		*/
		vec E_der(const Mat<double>& /* current q */);			// calculating the derivite of the current energy
		Mat<double> E_der_der(const Mat<double>& /* current q */);	//calculating the hessian matrix of current energy
		...
		double calculate_energy(const Mat<double>& /* current q */ );
